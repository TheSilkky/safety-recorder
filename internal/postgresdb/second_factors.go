...
func (r *Repository) GetActiveEmailSecondFactor(ctx context.Context, accountID string) (auth.SecondFactor, error) {
	row := r.db.QueryRowContext(ctx, `
		...
		WHERE account_id = $1 AND factor_type = $2 AND factor_state = $3`,
		accountID,
...
func (r *Repository) CreateActiveEmailSecondFactorChallenge(ctx context.Context, accountID string, expiresAt time.Time) (auth.SecondFactorChallenge, string, error) {
	...
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return auth.SecondFactorChallenge{}, "", fmt.Errorf("begin postgres active email second factor challenge: %w", err)
	}
	defer tx.Rollback()
	...
	if err != nil {
		...
		return auth.SecondFactorChallenge{}, "", fmt.Errorf("insert postgres active email second factor challenge: %w", err)
	}

	if err := tx.Commit(); err != nil {
		return auth.SecondFactorChallenge{}, "", fmt.Errorf("commit postgres active email second factor challenge: %w", err)
	}
	return challenge, rawToken, nil
}
