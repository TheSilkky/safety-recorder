...
| Regional relay | Health/readiness routes, configured backend-issued upload and fanout capabilities for authorized open streams, service-authenticated core relay preflight/commit/fanout authorization endpoints, configured complete-chunk upload forwarding with temporary ciphertext staging, and optimistic encrypted fanout marked unconfirmed. | Optional temporary, ciphertext-only relay listener subordinate to the core API, with backend confirmation/rejection propagation, metrics, production service identity, and deployment hardening only after separate review. |
...
Relay upload and fanout capabilities are not raw account sessions, browser
cookies, viewer tokens, incident tokens, raw keys, wrapped-key ciphertext,
uploaded bytes, stored paths, object keys, plaintext, or user safety data. They
must be treated as bearer-like credentials and kept out of logs, metrics
labels, public issues, and support artifacts.

Relay-to-core service auth is also separate from user sessions and relay upload
or fanout capabilities. The current static service token authorizes only the
narrow core preflight/commit/fanout authorization route set and must be kept
out of logs, metrics labels, public issues, and support artifacts.

Relay-local staged chunks are not backend-confirmed evidence. Near-live relay
fanout chunks are also not backend-confirmed evidence; trusted clients must
label relay-fanned chunks as unconfirmed until the core API commits encrypted
bytes and metadata.
