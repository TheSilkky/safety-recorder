...
| Public registration and required setup | Disabled by default; open mode requires SMTP verification; paid mode fails closed. New admin-created and open-registration accounts carry required setup state that blocks main product routes until email challenge setup is completed. | Explicit preview deployments may enable open registration with email verification, rate limits, deployment controls, account-scoped committed blob quota, stronger second-factor options, and a reviewed recovery policy. |
...
- email challenge second-factor setup before main product-route access
