...
| `SAFE_MAIN_API_RATE_LIMIT_ACCOUNT` | `120` | Account self-service, owner account/device recipient-key metadata, and owner contact public-key metadata requests allowed per window per hashed socket peer. Set to `0` to disable this route-class limit. |
