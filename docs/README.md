...
| [Regional stream ingress relay](regional-stream-ingress-relay.md) | Current health/readiness skeleton, backend-issued upload capabilities, service-authenticated core preflight/commit endpoints, and planning boundary for a future optional regional upload-only relay that stages ciphertext temporarily and lets the core API remain authoritative for authorization, durable commits, idempotency, and metadata. |
